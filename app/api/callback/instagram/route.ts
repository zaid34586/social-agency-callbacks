...
const REDIRECT_URI = 'https://social-agency-callbacks-jxb3hz0w7-match-mind-ai-s-projects.vercel.app/api/callback/instagram'
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const tokenResp = await fetch(
      ...
      `&redirect_uri=${encodeURIComponent('https://social-agency-callbacks-jxb3hz0w7-match-mind-ai-s-projects.vercel.app/api/callback/instagram')}` +
      `&code=${code}`
    )
    ...
    for (const page of pagesData.data || []) {
      ...
      if (igData.instagram_business_account) {
        console.log('Save Instagram token for client:', { 
          pageId: page.id, 
          pageName: page.name, 
